  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
      ...
      <p className="text-slate-500 text-sm line-clamp-2 mb-6 h-10">
        {quiz.description || "未提供描述。"}
      </p>
      <div className="flex flex-wrap gap-2 items-center">
        <Button variant="primary" size="sm" onClick={() => onStart(quiz.id)}>开始测试</Button>
        <Button variant="secondary" size="sm" onClick={() => onRandomStart(quiz.id)}>随机抽取</Button>
        <Button variant="secondary" size="sm" onClick={() => onShare(quiz.id)}>分享密钥</Button>
        <Button 
          variant="ghost" 
          size="sm" 
